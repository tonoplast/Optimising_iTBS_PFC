...
function output = peak_detection_negative(data,time,srate,peaks,peak_window,peak_replace)
...
% % % it will pick up the mininum value


        elseif size(latHold,1) > 1;
%           data.(peak_name).lat = latHold(1,:); % 1st peak
            [m i] = min(data.data(timezero+latHold)); %     
