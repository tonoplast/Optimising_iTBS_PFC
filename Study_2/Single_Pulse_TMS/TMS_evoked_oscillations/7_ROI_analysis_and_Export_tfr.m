    ...
    outPath = 'F:\Data\EEG\1_Intensity\ROI\Freq_analysis\stats\';
    Sesh = {'50'; '75'; '100'};
    tp = {'T0';'T1';'T2'};
...
for ee = 1:size(elec,1)
   ...
   for f = 1:size(FOI,1)
    ...
    elseif FOI{f,1} == [8,12]
        FOIName = FOINames{2};
        TOI = [0.050,0.250]; 
    elseif FOI{f,1} == [12,30]
        FOIName = FOINames{3};
        TOI = [0.050,0.250];
    elseif FOI{f,1}  == [30,45]
        FOIName = FOINames{4};
        TOI = [0.025,0.125];
    end
...
load([inPath filesep 'TMSEEG_' Sesh{y,1} '_final_' tp{z,1} '_ftWaveBc_ga']);
...
xlswrite([outPath filesep 'TBS_Intensity_Freq_' ROI '_' FOIName '.xlsx'],RSfreqOutputM,elec{ee,1},'B2');
xlswrite([outPath filesep 'TBS_Intensity_Freq_' ROI '_' FOIName '.xlsx'],RScol,elec{ee,1},'B1');
xlswrite([outPath filesep 'TBS_Intensity_Freq_' ROI '_' FOIName '.xlsx'],RSrow,elec{ee,1},'A2');

xlswrite([outPath filesep 'TBS_Intensity_Freq_' ROI '_' FOIName '.xlsx'],RSfreqOutput3M,strjoin(elecs, '_'),'B2');
xlswrite([outPath filesep 'TBS_Intensity_Freq_' ROI '_' FOIName '.xlsx'],RScol,strjoin(elecs, '_'),'B1');
xlswrite([outPath filesep 'TBS_Intensity_Freq_' ROI '_' FOIName '.xlsx'],RSrow,strjoin(elecs, '_'),'A2');

 clearvars iter1 iter3 freqOutput freqOutput3 freqOutputM freqOutput3M
