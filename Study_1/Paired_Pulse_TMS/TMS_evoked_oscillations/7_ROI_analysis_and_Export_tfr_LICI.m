    ...
    % output for each individual is given as the variable freqOutput in the
    % workspace. Each row represents the value for an individual (in the order
    % they were entered in to the grand average script).
    
    % This is for Long-interval Intracortical Inhibition (LICI)
...
load([inPath filesep 'PPTMS_' Sesh{y,1} '_LICI_' tp{z,1} '_ftWaveBc_ga']);
...
xlswrite([outPath filesep 'TBS_Freq_LICI_' ROI '_' FOIName '.xlsx'],RSfreqOutputM,elec{ee,1},'B2');
xlswrite([outPath filesep 'TBS_Freq_LICI_' ROI '_' FOIName '.xlsx'],RScol,elec{ee,1},'B1');
xlswrite([outPath filesep 'TBS_Freq_LICI_' ROI '_' FOIName '.xlsx'],RSrow,elec{ee,1},'A2');

xlswrite([outPath filesep 'TBS_Freq_LICI_' ROI '_' FOIName '.xlsx'],RSfreqOutput3M,strjoin(elecs, '_'),'B2');
xlswrite([outPath filesep 'TBS_Freq_LICI_' ROI '_' FOIName '.xlsx'],RScol,strjoin(elecs, '_'),'B1');
xlswrite([outPath filesep 'TBS_Freq_LICI_' ROI '_' FOIName '.xlsx'],RSrow,strjoin(elecs, '_'),'A2');
